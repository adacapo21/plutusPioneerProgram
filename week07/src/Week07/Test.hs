...
-- tests each of the four combinations
test :: IO ()
test = do
    test' Zero Zero
    test' Zero One
...
-- runEmulatorTraceIO’ variant
-- which allows us to set up the initial wallet distributions using an EmulatorConfig
test' :: GameChoice -> GameChoice -> IO ()
test' c1 c2 = runEmulatorTraceIO' def emCfg def $ myTrace c1 c2 -- c1, c2 represent choices
  where
...
myTrace c1 c2 = do
    ...
    -- start two instances of the contract, one for wallet 1 and one for wallet 2.
    h1 <- activateContractWallet (Wallet 1) endpoints
    h2 <- activateContractWallet (Wallet 2) endpoints
    -- look up the two public key hashes.
    let pkh1      = pubKeyHash $ walletPubKey $ Wallet 1
        ...
        fp = FirstParams
                ...
                , fpNonce          = "SECRETNONCE"      -- HARDCODED NONCE
